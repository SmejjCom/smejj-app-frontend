...
import { holeSendepfad } from "./sendepfad-nachladen.js?v=20";
...
function goToView(viewId, { replace = false } = {}) {
  ...
  ladeSucheFuerAnsicht(resolvedViewId); // Suche erst beim Oeffnen ihrer Ansicht
  updateCanonical();
...
function bindStartComposer() {
  ...
  ladeBeiKlick(["[data-start-tool]", "#composerPlusButton"], () => import("./composer-tools.js?v=werkzeuge-24").then((m) => m.initComposerTools()));
